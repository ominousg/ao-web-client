...
class Camera {
	...
	forEachVisibleNextLinea(direccion, callback, extraPositions, offsetX, offsetY) {
	...
	isVisiblePosition(gridX, gridY, extraPositions) {
		...
		let extraXEste = extraPositions.este,
			...
			extraYNorte = extraPositions.norte;
		if (
			gridY >= this.gridY - extraYNorte &&
			gridY < this.gridY + this.gridH + extraYSur &&
			gridX >= this.gridX - extraXOeste &&
			gridX < this.gridX + this.gridW + extraXEste
		) {
			return true;
		} else {
			return false;
	...
	rectVisible(rect, extraPositions) {
		// eje x,y en esquina izquierda superior de rect
		extraPositions = extraPositions || this.DEFAULT_EXTRA_POSITIONS;

		return !(
			this.x > rect.x + rect.width + +extraPositions.oeste * this.tilesize ||
