				...
				switch (comando) {
					...
					case '/ENCUESTA':
						if (args.length === 0) {
							...
							if (!isNaN(args[0]) && args[0] < 256) {
								...
								this.game.escribirMsgConsola(
									'Para votar una opción, escribe /ENCUESTA NUMERODEOPCION, por ejemplo para votar la opcion 1, escribe /encuesta 1.',
									Font.WARNING
								);
							}
						}
					...
					case '/CENTINELA':
						if (args.length) {
							if (!isNaN(args[0]) && args[0] < 65536) {
								...
								this.game.escribirMsgConsola(
									'El código de verificación debe ser numérico. Utilice /CENTINELA X, siendo X el código de verificación.'
								);
							}
						} else {
							this.game.escribirMsgConsola(
								'Faltan parámetros. Utilice /CENTINELA X, siendo X el código de verificación.'
							);
						}
						break;
					...
					case '/FUNDARCLAN':
						if (this.game.atributos.nivel > 24) {
							...
							this.game.escribirMsgConsola(
								'Para fundar un clan tienes que ser nivel 25 y tener 90 skills en liderazgo.'
							);
						}
						break;
					...
					case '/BANIP':
						if (args.length == 2) {
							...
							this.game.escribirMsgConsola(
								'Faltan parámetros. Utilice /BANIP IP MOTIVO o /BANIP NICK MOTIVO.'
							);
						}
						break;
