...
const clanes = ["La Logia", "TRAPO", "KenhoKai", "NAPALM", "Valhalla", "BlueSky", "Arcanos", "La Mazorka", "Irilend", "Los Intocables", "Austland",
  "Dha Mastha", "TalKinG", "BRUJERIA", "mRaiders", "Gauchos del Cielo", "IroN PoweR", "No MerCy", "Clase A", "Caradrim", "KarmianS",
  "Caballeros Del Temple", "M A F I A", "Eldar Ithil", "Praetorians", "HellFish", "Hay Equipo", "Hijos de Marco Apostol", "Gefolgen",
  "La Granja", "Zoldick", "Lumiere", "attitudE", "Sword Knights", "Zoltan", "Hijos De Belcebu", "The Clash", "VenitA", "The PoweR"];
...
const ClanesList = () => {
  ...
  const filteredResults = useMemo(() => {
    if (debouncedSearchInput.length >= 1) {
      return clanes.filter(clan =>
        clan.toLowerCase().includes(debouncedSearchInput.toLowerCase())
      );
    }
    return clanes;
      ...
      <div css={clanesPopup}>
        <div css={sidebarStyle}>
          {tabs.map((tab) => (
            ...
            >
              <img
                src={activeTab === tab.id ? `../imagenes/interfaz/iconos/${tab.icon}_active.png` : `../imagenes/interfaz/iconos/${tab.icon}.png`}
                alt={tab.label}
              />
            </button>
          ))}
